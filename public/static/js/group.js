layui.define(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'form'], function (exports) {
    ...
    table.render({
        ...
        cols: [[ //表头
            {field: 'group_name', title: '组名', align: 'center'},
    ...
    table.on('tool(table_list)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') {
            operation_handle(data['group_id'], 'view');
            ...
            operation_handle(data['group_id'], 'edit');
        }
    });
    ...
    function operation_handle(id, type) {
        layer.open({
            ...
            content: '<iframe style="height: 98%; width: 100%; border: 0;" src="../group/form/?id=' + id + '&type=' + type + '" />',
            area: ['60%', '80%'],
            maxmin: true,
            btn: ['关闭'],
            yes: function (index) {
