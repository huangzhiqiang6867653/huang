layui.define(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'form'], function (exports) {
    ...
    table.render({
        ...
        cols: [[ //表头
            {field: 'company_id', title: '主键', sort: true, align: 'center'},
            {field: 'company_name', title: '公司名', align: 'center'},
            {field: 'address', title: '地址', align: 'center'},
            {field: 'contacts', title: '联系人', align: 'center'},
            {field: 'contact_number', title: '联系电话', align: 'center'},
    ...
    table.on('tool(table_list)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') {
            operation_company(data['company_id'], 'view');
            ...
            operation_company(data['company_id'], 'edit');
        }
    });
    ...
    $(document).on('click', '#add_operation_id', function () {
        operation_company('', 'add')
    });

    function operation_company(id, type) {
        layer.open({
            title: type == 'add' ? '新增' : (type == 'edit' ? '修改' : '查看'),
            content: '<iframe style="height: 98%; width: 100%; border: 0;" src="../company/form/?id=' + id + '&type=' + type + '" />',
            area: ['60%', '80%'],
            maxmin: true,
            btn: ['关闭'],
            yes: function (index) {
